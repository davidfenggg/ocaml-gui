...
let make_controller (v: 'a) : 'a value_controller =
  let r = { contents : ('a -> unit) list = [] } in
  let va = { contents : 'a = v } in
  {
    add_change_listener = 
      (fun x -> r.contents <- x :: r.contents);
    get_value = (fun () -> va.contents);
    change_value = (fun x -> va.contents <- x; 
      let rec loop (listeners: ('a -> unit) list) (list_val: 'a) =
        begin match listeners with
        | [] -> ();
        | h::t -> h list_val; loop t list_val;
        end
      in loop r.contents va.contents;)
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
 (
  let (w_lab, l_lab) = label s in 
  let v_controller = make_controller init in
  let w_final = 
  {
  repaint = (fun (g: Gctx.gctx) -> 
    Gctx.draw_rect g (0,0) (20,20);
    if v_controller.get_value() 
      then (Gctx.draw_line g (0,0) (20,20); Gctx.draw_line g (20,0) (0,20));
    let g' = Gctx.translate g (30,0) in
    w_lab.repaint g');
  handle = (mouseclick_listener (fun () -> v_controller.change_value (not 
  (v_controller.get_value()))));
  
  (*
  (fun (g:Gctx.gctx) (e:Gctx.event) -> 
    let (x,y) = w_lab.size() in
    let tf = v_controller.get_value() in
    if Gctx.event_type e = Gctx.MouseDown && event_within g e 
    (30 + x, max y 20) 
      then v_controller.change_value (not tf)
   );
  *)
  size = (fun () -> 
    let (x,y) = w_lab.size() in
...
let slider (c: Gctx.color) (init: int) (max: int) : 
...
(
  ...
  {
    repaint = (fun (g: Gctx.gctx) -> 
    Gctx.draw_rect g (0,0) (200,20);
    let init_float = Float.of_int (v_controller.get_value()) in
    let max_float = Float.of_int max in
    let percent_float = Float.div init_float max_float in
    let fill_percent = Float.mul percent_float 200.0 in
    let percent = Float.to_int fill_percent in
    let g' = Gctx.with_color g c in
    Gctx.fill_rect g' (1,1) (percent, 18)
    );
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
    if event_within g e (200, 20) && (Gctx.event_type e = Gctx.MouseDown ||
    Gctx.event_type e = Gctx.MouseDrag) then
    let p = Gctx.event_pos e g in
    let init_float = Float.of_int (fst p) in
    let max_float = Float.of_int max in
    let percent_float = Float.div init_float 200.0 in
    let fill_percent = Float.mul percent_float max_float in
    let percent = Float.to_int fill_percent in
    v_controller.change_value percent;
    );
    size = (fun () -> (200,20))
  } in
  w_final,
